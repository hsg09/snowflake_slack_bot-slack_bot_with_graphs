...
class ToolResult:
    ...
    @property
    def chart_data(self) -> Optional[Dict[str, Any]]:
        """Extract chart data from tool results (from data_to_chart tool)."""
        for item in self.content:
            if isinstance(item, dict):
                # Check for direct chart fields
                if 'chart' in item:
                    return item['chart']
                if 'charts' in item:  # Handle plural 'charts'
                    return item['charts']
                if 'image' in item:
                ...
                if 'image_base64' in item:
                    return {'image_base64': item['image_base64']}
                ...
                if 'json' in item:
                    ...
                    if 'charts' in json_data:  # Handle plural 'charts'
                        return json_data['charts']
                    if 'image' in json_data:
                        return {'image': json_data['image']}
                    if 'image_url' in json_data:
                        return {'image_url': json_data['image_url']}
                    if 'image_base64' in json_data:
                        return {'image_base64': json_data['image_base64']}
                    if 'chart_url' in json_data:
                        return {'chart_url': json_data['chart_url']}
                    if 'visualization' in json_data:
                        return json_data['visualization']
                        
                # Check for text content that might contain base64 image
                if 'text' in item:
                    text = item['text']
                    if isinstance(text, str) and (text.startswith('data:image') or text.startswith('iVBOR')):
                        return {'image_base64': text}
        return None
    
    @property
    def result_set(self) -> Optional[Dict[str, Any]]:
        """Extract result_set (tabular data) from tool results.
        
        Returns a dict with 'data' (the rows) and optionally 'columns' (column names).
        """
        for item in self.content:
            if isinstance(item, dict):
                result_data = None
                ...
                if 'result_set' in item:
                    raw_result_set = item['result_set']
                if 'columns' in item:
                ...
                if 'json' in item:
                    ...
                    if 'result_set' in json_data:
                        raw_result_set = json_data['result_set']
                    if 'columns' in json_data:
                        columns = json_data['columns']
                    if 'schema' in json_data:
                        columns = json_data['schema']
                
                # Handle Snowflake result set structure with resultSetMetaData
                if isinstance(raw_result_set, dict):
                    # Extract data from Snowflake format
                    if 'data' in raw_result_set:
                        result_data = raw_result_set['data']
                    
                    # Extract column names from resultSetMetaData
                    if 'resultSetMetaData' in raw_result_set and not columns:
                        meta = raw_result_set['resultSetMetaData']
                        if 'rowType' in meta:
                            # rowType contains column definitions
                            columns = [col.get('name', f'col_{i}') for i, col in enumerate(meta['rowType'])]
                else:
                    result_data = raw_result_set
...
class CortexResponse:
    ...
    def charts(self) -> List[Dict[str, Any]]:
        """Extract all chart data from tool results."""
        all_charts = []
        for message in self.messages:
            for tool_result in message.tool_results:
                chart = tool_result.chart_data
                if chart:
    ...
    def result_sets(self) -> List[Dict[str, Any]]:
        """Extract all result_sets (tabular data) from tool results.
        
        Returns list of dicts, each with 'data' and optionally 'columns'.
        """
        all_results = []
        for message in self.messages:
            for tool_result in message.tool_results:
                result_set = tool_result.result_set
                if result_set:
...
class CortexResponseParser:
        ...
        for line in sse_lines:
            ...
            if line.startswith('data: '):
                ...
                try:
                    ...
                    # Handle tool result events (contains SQL queries, verification info, and charts)
                    elif current_event == 'response.tool_result':
                        if self.debug:
                            print(f"\n📦 PARSER: response.tool_result event")
                            print(f"   tool_use_id: {json_data.get('tool_use_id', 'unknown')}")
                            print(f"   content type: {type(json_data.get('content'))}")
                            if 'content' in json_data:
                                content_list = json_data['content']
                                if isinstance(content_list, list):
                                    for idx, item in enumerate(content_list):
                                        if isinstance(item, dict):
                                            print(f"   content[{idx}] keys: {item.keys()}", flush=True)
                                            # Log any image/chart data found
                                            for key in ['image', 'image_base64', 'image_url', 'chart', 'charts', 'json', 'text', 'result_set']:
                                                if key in item:
                                                    val = item[key]
                                                    if isinstance(val, str) and len(val) > 100:
                                                        print(f"   content[{idx}].{key}: {val[:100]}... (len={len(val)})", flush=True)
                                                    elif isinstance(val, dict):
                                                        print(f"   content[{idx}].{key} keys: {val.keys()}", flush=True)
                                                    elif isinstance(val, list) and len(val) > 0:
                                                        print(f"   content[{idx}].{key}: list[{len(val)}], first item type: {type(val[0])}", flush=True)
                                                        if isinstance(val[0], dict):
                                                            print(f"   content[{idx}].{key}[0] keys: {val[0].keys()}", flush=True)
                                                    else:
                                                        print(f"   content[{idx}].{key}: {val}", flush=True)
    ...
    def extract_summary(self, response: CortexResponse) -> Dict[str, Any]:
        ...
        return {
            ...
            'charts': response.charts,  # Add chart data
            'result_sets': response.result_sets  # Add tabular result data
        }
